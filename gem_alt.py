...
GREEN, AMBER = (46, 204, 113), (243, 156, 18)
FONT = "inter,segoeui,consolas"

# ----------------------------------------------------------------------------
# Layout grid
# ----------------------------------------------------------------------------
SCREEN_W, SCREEN_H = 980, 800
MARGIN = 50
BOX_W, BOX_H = 200, 50
COL_GAP, ROW_GAP = 20, 20
COL_X = [MARGIN, MARGIN + BOX_W + COL_GAP]

TAB_W, TAB_H, TAB_GAP = 180, 30, 16
_tabs_total = 2 * TAB_W + TAB_GAP
TAB_X = [(SCREEN_W - _tabs_total) // 2 + i * (TAB_W + TAB_GAP) for i in range(2)]
TABS_Y = 8
TABS_RULE_Y = TABS_Y + TAB_H + 4
...
# Charts
# ----------------------------------------------------------------------------
def stock_graph(ticker_str, dates, values, n, w, h):
    try:
        if not dates:
...
def fcf_graph(past_fcf_series, proj_fcf, n, w, h):
    try:
        past_pairs = []
...
def RiskData(ticker_str):
    ...
    ann_ret = float(hist.iloc[-1] / hist.iloc[0] - 1)
    max_dd = float((hist / hist.cummax() - 1).min())
    rf = risk_free_rate()
    sharpe = (ann_ret - rf) / ann_vol if ann_vol else 0.0
    b = beta if beta is not None else ann_vol / 0.2
...
def WACC(ticker_str):
    ...
    cost_equity = risk_free_rate() + 0.05 * t.info.get("beta", 1.0)
...
def DCF(proj_time_1, growth_r_1, p_growth_r_1, wacc_2, ebit_1, ebitda_1, ncwc_1_a, ncwc_1_b,
    ...
    # Guardrail to prevent infinite terminal values
    if wacc_2 <= p_growth_r_1:
        return "Error: WACC must be strictly greater than Perpetuity Growth Rate.", []
    ...
    def grow(current_val, base_val, rate):
        """Standard compound growth for positive base values.
        Linear step improvement for negative values to organically cross into profitability."""
        if base_val >= 0:
            return current_val * (1 + rate)
        else:
            return current_val + abs(base_val) * rate

    for i in range(1, int(proj_time_1) + 1):
        # Progressions using the helper logic
        ebit_2.append(grow(ebit_2[i - 1], ebit_2[0], growth_r_1))
        d_a.append(grow(d_a[i - 1], d_a[0], growth_r_1))
        cap_ex_2.append(grow(cap_ex_2[i - 1], cap_ex_2[0], growth_r_1))

        # Prevent division by zero and wild swings for unprofitable companies crossing $0
        if ebit_2[0] > 0 and ebit_2[i - 1] != 0:
            ncwc_b.append((ncwc_b[i - 1] / ebit_2[i - 1]) * ebit_2[i])
            tax_2.append((tax_2[0] / ebit_2[0]) * ebit_2[i])
        else:
            ncwc_b.append(grow(ncwc_b[i - 1], ncwc_b[0], growth_r_1))
            tax_2.append(grow(tax_2[i - 1], tax_2[0], growth_r_1))

        ncwc_c.append(ncwc_b[i] - ncwc_b[i - 1])
        fcf.append(ebit_2[i] - tax_2[i] + cap_ex_2[i] - ncwc_c[i] + d_a[i])
        present_value.append(fcf[i] / ((1 + wacc_2) ** i))

    term_value = (fcf[-1] * (1 + p_growth_r_1)) / (wacc_2 - p_growth_r_1)
    ent_val = term_value / (1 + wacc_2) ** int(proj_time_1) + sum(present_value[1:])
    equ_val = ent_val + total_cash_1 - total_debt_1
    dis_stock_p = equ_val / s_out_1

    val_percent = (dis_stock_p - c_share_p) / c_share_p * 100 if c_share_p > 0 else 0
...
def FMajor(ticker_str, proj_time_0, growth_r_0, p_growth_r_0, wacc_1):
    ...
    stock_p_0 = float(t.history(period="1d")['Close'].iloc[-1])
    ...
    past_fcf_series = get_series(c_flow, ['Free Cash Flow'])
...
def draw_button(rect, label, fill, border, text_col, font_size=15):
...
def inp_box(x, y, w, h, text, font_size=15):
...
def ui(screen_, mainClock):
    ...
    def draw_tabs():
        ...
        for (label, mode), x in zip([("DCF Analysis", 'dcf'), ("Risk Profile", 'risk')], TAB_X):
            ...
            rects[mode] = r
        pygame.draw.line(screen, BORDER, (0, TABS_RULE_Y), (SCREEN_W, TABS_RULE_Y), 1)
        return rects

    def draw_beta_gauge(x, y, w, d):
    ...
    def draw_dcf_view():
        ...
        pygame.draw.line(screen, BORDER, (490, TABS_RULE_Y), (490, SCREEN_H), 1)
        ...
        out_font = pygame.font.SysFont(FONT, 15)
        ...
        rects = [inp_box(COL_X[0], row_y(0), BOX_W, BOX_H, text[0]),
                 ...
                 wacc_box(),
                 inp_box(COL_X[0], row_y(3), out_w, out_h, text[5])]

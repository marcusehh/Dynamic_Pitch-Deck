...
def WACC(ticker_str): #Automatically derives a WACC for the DCF
    ...
    i_state = t.incomestmt.T.fillna(0) #Income statement as a table, NaN -> 0
    ...
    cost_debt = abs(i_state.get('Interest Expense', 0).iloc[0]) / total_debt #Magnitude / total debt
...
def FMajor(ticker_str, proj_time_0, growth_r_0, p_growth_r_0, wacc_1):
    ...
    ncwc = ((b_sheet.get('Total Current Assets', 0) - b_sheet.get('Cash And Cash Equivalents', 0) -
             ...
             b_sheet.get('Current Debt', 0))) #Non-cash working capital

    stock_p_0 = t.history(period="1d")['Close'] #Most recent close
    total_cash_0 = t.info.get('totalCash', 0)
    total_debt_0 = t.info.get('totalDebt', 0)
    s_out_0 = t.info.get('sharesOutstanding', 0)

    dcf_string, proj_fcf_list = DCF(proj_time_0, growth_r_0, p_growth_r_0, wacc_1, ebit_0.iloc[0],
                                    ebitda_0.iloc[0], ncwc.iloc[1], ncwc.iloc[0], cap_ex_0.iloc[0],
                                    tax_0.iloc[0], stock_p_0.iloc[0], s_out_0, total_cash_0,
                                    total_debt_0, ticker_str)
    past_fcf_series = c_flow.get('Free Cash Flow') #Historical FCFs
